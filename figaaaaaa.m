...
img_name = "22_training.tif";
img_path = img_folder + img_name;
image = im2double(imread(img_path));

mask_name = "DRIVE_Dataset\training\mask\22_training_mask.gif";
mask = imread(mask_name);
mask = imbinarize(mask);
scrtele = strel('disk', 15);
mask = imerode(mask, scrtele);


GT_name = "DRIVE_Dataset\training\1st_manual\22_manual1.tif";
GT = imread(GT_name);
GT = imbinarize(GT);
...
subplot(1,2,1)
imshow(fusedImg ,[])
title('Modified Top-Hat Result', 'FontSize', 35)
subplot(1,2,2)
imshow(frangiImg, [])
title('Frangi Filter Result', 'FontSize',35)

imageArray = reshape(frangiImg, 1, []);
tfrangi = triangleThreshold(imageArray,128);
segImg = imbinarize(frangiImg, tfrangi);
segImg = bwareaopen(segImg, 250) & mask;
